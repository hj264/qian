...
export class APIClientConfig {
    private _apiURL: string = STATIC.BASE_URL
...
export default class APIClient {
    ...
    post(path: string, data?: any, config?: RequestConfig) {
        if (path.includes("SocketAddress")) {
            showLoadingToast({
                message: 'Loading...',
                forbidClick: true,
                duration: 1200
    ...
    joinChannel = (channelID: string, channelType: number, uid: string) => {
        APIClient.shared.post(STATIC.BASE_URL + '/channel/subscriber_add', {
    ...
    syncMessages = async (channel: Channel, opts: SyncOptions) => {
        ...
        const limit = 30;
        const resp = await APIClient.shared.post(STATIC.BASE_URL + '/wsapi/channel/messagesync', {
    ...
    syncConversations = async () => {
        ...
        const conversationList = await APIClient.shared.post(STATIC.BASE_URL + '/wsapi/conversation/sync', {
    ...
    clearUnread = async (channel: Channel) => {
        return APIClient.shared.post(STATIC.BASE_URL + '/wsapi/conversations/setUnread', {
            uid: WKSDK.shared().config.uid,
            channel_id: channel.channelID,
            channel_type: channel.channelType,
            unread: 0,
    ...
    sendCMD = async (channel: Channel, cmd: string, param?: any) => {
        ...
        return APIClient.shared.post(STATIC.BASE_URL + '/wsapi/message/send', {
            "header": {// 消息头
    ...
    messageStreamStart = (param: any) => {
        return APIClient.shared.post(STATIC.BASE_URL + '/wsapi/streammessage/start', param)
    }
    messageStreamEnd = (param: any) => {
        return APIClient.shared.post(STATIC.BASE_URL + '/wsapi/streammessage/end', param)
    }
}
